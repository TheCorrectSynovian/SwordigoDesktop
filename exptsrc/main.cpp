...
#if 1 // Assistant-added SRE ButtonController shim — commented out per user request
struct SreBtnSlot {
...
void load_and_boot_arm64() {
    ...
    //
    // Version restriction removed — libsre.so loads for any ARM64 instance
    // that has it configured in the manifest. The user decides compatibility.
    {
        bool sre_compatible = (g_lib_name.find("arm64") != std::string::npos);
        
        if (!sre_compatible) {
            std::cerr << "\n============================================" << std::endl;
            std::cerr << "  [SRE] Skipped — ARM64 binary required" << std::endl;
        ...
        if (!sre_path.empty() && access(sre_path.c_str(), F_OK) == 0) {
            ...
            if (sre_ret == 0) {
                ...
                if (sre_btn_array_addr) {
                    ...
                    // Quick dump of active buttons for debugging
                    SreBtnSlot* dbg_btn = nullptr;
                    for (int i = 0; i < SRE_BTN_MAX; ++i) {
                        dbg_btn = (SreBtnSlot*)(g_guest_memory + sre_btn_array_addr + i * sizeof(SreBtnSlot));
                        if (dbg_btn && dbg_btn->active) {
                            std::string id_str(dbg_btn->id);
                            std::string label_str(dbg_btn->label);
                            std::cout << "[SRE Button] slot=" << i << " id='" << id_str
                                      << "' label='" << label_str << "' x=" << dbg_btn->cur_x
                                      << " y=" << dbg_btn->cur_y << std::endl;
                        }
                    }
                } else {
                    std::cout << "[SRE] ButtonController symbols not found in guest — g_sre_buttons unresolved" << std::endl;
                }
    ...
    if (updateApp && drawApp) {
        ...
        while (running) {
            ...
            if (g_display_active) {
                ...
                // ---- ButtonController: auto-hide during menus + overlay ----
                // (Commented out assistant-added ButtonController integration)
                /*
                ...
                while (SDL_PollEvent(&event)) {
                    switch (event.type) {
                        ...
                        case SDL_EVENT_MOUSE_BUTTON_DOWN:
                            if (event.button.button == SDL_BUTTON_LEFT) {
                                ...
                                if (gui_consumed) {
                                    ...
                                    // (Commented out assistant-added ButtonController hit-test and debug logging)
                                    /*
                                    bool btn_hit = false;
                                    if (sre_btn_array_addr) {
                                        ...
                                        if (!globally_hidden) {
                                            for (int i = SRE_BTN_MAX - 1; i >= 0; i--) {
                                                ...
                                                if (mx >= bx && mx <= bx + pw && my >= by && my <= by + ph) {
                                                    ...
                                                    btn_hit = true;
                                                    // Debug: log button press (avoid huge spam by only logging on transition)
                                                    if (btn->id[0]) {
                                                        std::string id_s(btn->id);
                                                        std::string label_s(btn->label);
                                                        std::cout << "[SRE Button] DOWN slot=" << i << " id='" << id_s
                                                                  << "' label='" << label_s << "'" << std::endl;
                                                    }
                                    ...
                                    */
                                    // Fallback: always send touch to guest (previous behavior)
                                    click_swallowed_by_gui = false;
                                    {
                                        float x = event.button.x * 960.0f / (float)g_win_w;
                                        float y = 544.0f - (event.button.y * 544.0f / (float)g_win_h);
                        ...
                        case SDL_EVENT_MOUSE_MOTION:
                            mouse_x = event.motion.x;
                            mouse_y = g_win_h - event.motion.y;
                            // ButtonController drag tracking (commented out assistant changes)
                            /*
                            if (sre_btn_array_addr) {
                                for (int i = 0; i < SRE_BTN_MAX; i++) {
                                    SreBtnSlot* btn = (SreBtnSlot*)(g_guest_memory + sre_btn_array_addr + i * sizeof(SreBtnSlot));
                        ...
                        case SDL_EVENT_MOUSE_BUTTON_UP:
                            if (event.button.button == SDL_BUTTON_LEFT) {
                                ...
                                mouse_x = event.button.x;
                                mouse_y = g_win_h - event.button.y;
                                // ButtonController release + snapback (commented out assistant changes)
                                /*
                                if (sre_btn_array_addr) {
                                    for (int i = 0; i < SRE_BTN_MAX; i++) {
                                        SreBtnSlot* btn = (SreBtnSlot*)(g_guest_memory + sre_btn_array_addr + i * sizeof(SreBtnSlot));
                                        ...
                                        if (btn->pressed) {
                                            ...
                                            if (btn->id[0]) {
                                                std::string id_s(btn->id);
                                                std::string label_s(btn->label);
                                                std::cout << "[SRE Button] RELEASE slot=" << i << " id='" << id_s
                                                          << "' label='" << label_s << "'" << std::endl;
                                            }
                                        }
                                        btn->dragging = 0;
