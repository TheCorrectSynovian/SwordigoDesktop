...
SreHookEntry sre_hook_table[] = {
     ...
     * Hook RegisterProgramLibrary to inject Mini.* as early as possible.
     * Disabled: this hook caused early replacement/injection that broke
     * engine-side initialization in some builds. Revert to lazy injection.
     */
    /* { 0x4c0f18, "sre_RegisterProgramLibrary", 0 }, */
    /* Hook luaL_newstate symbol to capture and inject immediately when
     * a new Lua state is created. The host resolves the actual symbol offset
     * at runtime; set target_offset=0 to indicate symbol lookup.
     * Disabled — early luaL_newstate hooking caused instability. */
    /* { 0, "sre_luaL_newstate", 0 }, */
